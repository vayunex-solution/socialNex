...
function LandingPage() {
    ...
    // Show theme picker popup only on first visit ever
    const [showThemePopup, setShowThemePopup] = useState(false)
    useEffect(() => {
        const seen = localStorage.getItem('theme-popup-seen')
        if (!seen) {
            // small delay so page loads first
            const t = setTimeout(() => setShowThemePopup(true), 800)
            return () => clearTimeout(t)
        }
    }, [])
    const handleThemeChoice = (choice) => {
        if (choice !== null) {
            // if current theme doesn't match the choice, toggle it
            if (theme !== choice) toggleTheme()
        }
    ...
    return (
        <div className="lp">
            ...
            {/* ═══════ FIRST-VISIT THEME PICKER POPUP ═══════ */}
            {showThemePopup && (
                <div className="lp-theme-popup-overlay" onClick={() => handleThemeChoice(null)}>
                    <div className="lp-theme-popup" onClick={e => e.stopPropagation()}>
                        <div className="lp-theme-popup-icon">👁️</div>
                        <h3 className="lp-theme-popup-title">Choose Your Vibe</h3>
                        <p className="lp-theme-popup-sub">How do you like it?</p>
                        <div className="lp-theme-popup-options">
                            <button
                                className={`lp-theme-option ${theme === 'dark' ? 'active' : ''}`}
                                onClick={() => handleThemeChoice('dark')}
                            >
                                <Moon size={28} />
                                <span>Dark Mode</span>
                                <small>Cosmic Purple</small>
                            </button>
                            <button
                                className={`lp-theme-option ${theme === 'light' ? 'active' : ''}`}
                                onClick={() => handleThemeChoice('light')}
                            >
                                <Sun size={28} />
                                <span>Light Mode</span>
                                <small>Clean & Crisp</small>
                            </button>
                        </div>
                        <button className="lp-theme-popup-skip" onClick={() => handleThemeChoice(null)}>Skip for now</button>
                    </div>
                </div>
            )}
            <SEO {...SEO_CONFIG} />
